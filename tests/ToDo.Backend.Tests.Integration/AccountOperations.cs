...
namespace ToDo.Backend.Tests.Integration
{
    public static class AccountOperations
    {
        ...
        {
            return $"{Guid.NewGuid():N}@company.com";
        }
        
        public static async Task<HttpResponseMessage> RegisterUserAsync(this HttpClient client,
            string email,
            string password)
        {
        ...
        public static async Task<HttpResponseMessage> LoginUserAsync(this HttpClient client,
            string email, 
            string password)
        {
